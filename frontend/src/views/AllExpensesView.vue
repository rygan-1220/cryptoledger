    ...
    <div class="max-w-6xl mx-auto">
      ...
      <div class="bg-surface border border-border rounded p-4 mb-6 flex gap-4 flex-wrap items-start">
        ...
        <div>
          ...
          <select v-model="filters.category" @change="fetch" class="border border-border rounded px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 transition">
      ...
      <div v-else-if="!expenses.length" class="bg-surface border border-border rounded p-16 text-center text-text-muted">No expenses found.</div>
        ...
        <table class="w-full text-sm">
          ...
          <tbody class="divide-y divide-border">
            <tr v-for="exp in expenses" :key="exp.expense_id" class="hover:bg-gray-50 transition">
              ...
              <td class="px-5 py-3 text-right font-medium">RM {{ parseFloat(exp.amount).toFixed(2) }}</td>
