<template>
  <div class="min-h-screen flex items-center justify-center bg-background">
    <div class="max-w-md w-full bg-surface p-8 rounded shadow-md border border-border">
      ...
      <form @submit.prevent="handleRegister" class="space-y-4">
        <div>
          ...
          <input v-model="form.username" type="text" class="w-full bg-background text-text-main border border-border rounded px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 transition" required />
        </div>

        <div>
          <label class="block text-xs font-bold text-text-muted uppercase mb-1">Email</label>
          <input v-model="form.email" type="email" class="w-full bg-background text-text-main border border-border rounded px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 transition" required />
        </div>

        <div>
          <label class="block text-xs font-bold text-text-muted uppercase mb-1">Password</label>
          <input v-model="form.password" type="password" class="w-full bg-background text-text-main border border-border rounded px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 transition" required minlength="8" />
        </div>

        <div>
          <label class="block text-xs font-bold text-text-muted uppercase mb-1">Department</label>
          <select v-model="form.dept_id" class="w-full bg-background text-text-main border border-border rounded px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 transition" required>
        ...
        <div class="bg-amber-50 border border-amber-400 rounded p-4 mt-4">
        ...
        <button type="submit" :disabled="loading" class="w-full bg-primary text-white font-medium py-2.5 mt-4 rounded hover:bg-primary-hover transition disabled:opacity-50">
