<template>
  <div class="min-h-screen bg-background flex flex-col justify-center py-12 px-6 lg:px-8">
    <div class="sm:mx-auto sm:w-full sm:max-w-3xl">
      ...
      <div class="bg-surface border border-border shadow-2xl rounded overflow-hidden">
        ...
        <div class="p-8 sm:p-12">
          ...
          <div v-if="step === 1" class="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
            ...
            <div class="space-y-4">
              <div>
                ...
                <input v-model="form.companyName" required class="w-full bg-background border border-border rounded px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50" placeholder="e.g. Acme Corp" />
              </div>
              <div>
                <label class="block text-xs font-bold text-text-muted uppercase mb-1">Workspace ID</label>
                <input v-model="form.workspaceId" required class="w-full bg-background border border-border rounded px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 font-mono text-primary" placeholder="acme-corp" />
              </div>
              <div class="grid grid-cols-2 gap-4 pt-4 border-t border-border">
                <div>
                  <label class="block text-xs font-bold text-text-muted uppercase mb-1">Database URL (PostgreSQL)</label>
                  <input v-model="form.dbUrl" required class="w-full bg-background border border-border rounded px-4 py-3 text-xs font-mono focus:outline-none focus:ring-2 focus:ring-primary/50" />
                </div>
                <div>
                  <label class="block text-xs font-bold text-text-muted uppercase mb-1">Redis URL</label>
                  <input v-model="form.redisUrl" required class="w-full bg-background border border-border rounded px-4 py-3 text-xs font-mono focus:outline-none focus:ring-2 focus:ring-primary/50" />
                </div>
              </div>
            </div>
            <div class="flex justify-end pt-4">
              <button @click="nextStep" :disabled="!form.companyName || !form.workspaceId" class="bg-primary text-white px-6 py-3 rounded font-bold hover:bg-primary-hover transition disabled:opacity-50">Next: Security & Config →</button>
            </div>
          </div>
          ...
          <div v-if="step === 2" class="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
            ...
            <div class="space-y-6">
              <!-- Session TTL -->
              <div class="p-5 bg-gray-50 border border-border rounded space-y-3">
                <div class="flex items-start gap-3">
                  <div class="p-2 bg-primary/10 rounded mt-0.5">
                ...
                <select v-model="form.sessionTtl" class="w-full bg-background border border-border rounded px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 transition">
              ...
              <!-- Rate Limiting -->
              <div
                class="p-5 border rounded cursor-pointer transition-all"
              ...
              >
                <div class="flex items-center justify-between">
                  <div class="flex items-start gap-3">
                    <div class="p-2 rounded mt-0.5" :class="form.enableRateLimit ? 'bg-primary/10' : 'bg-gray-200'">
            ...
            <div class="flex justify-between pt-4">
              ...
              <button @click="nextStep" class="bg-primary text-white px-6 py-3 rounded font-bold hover:bg-primary-hover transition">Next: Departments →</button>
            </div>
          </div>
          ...
          <div v-if="step === 3" class="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
            ...
            <div class="space-y-3">
              <div class="flex items-center gap-3 p-3 bg-gray-50 border border-border rounded">
              ...
              <div v-for="(dept, index) in form.departments" :key="index" class="flex items-center gap-3">
                <input v-model="form.departments[index]" class="flex-1 bg-background border border-border rounded px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50" placeholder="Department Name" />
                <button @click="removeDept(index)" class="p-3 text-ember hover:bg-red-50 rounded transition">
            ...
            <div class="flex justify-between pt-4">
              <button @click="prevStep" class="text-text-muted hover:text-text-main px-4 py-2 font-medium">← Back</button>
              <button @click="nextStep" :disabled="form.departments.some(d => !d.trim())" class="bg-primary text-white px-6 py-3 rounded font-bold hover:bg-primary-hover transition disabled:opacity-50">Next: Managers →</button>
            </div>
          </div>
          ...
          <div v-if="step === 4" class="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
            ...
            <div v-if="form.departments.length === 0" class="p-4 bg-blue-50 text-blue-700 rounded text-sm border border-blue-100">
              No custom departments created. You can proceed to Admin setup.
            </div>
            
            <div class="space-y-6 max-h-[400px] overflow-y-auto pr-2">
              <div v-for="(dept, index) in form.departments" :key="index" class="p-5 bg-surface border border-border rounded space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input v-model="form.managers[index].name" required class="w-full bg-background border border-border rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50" placeholder="Manager Name" />
                  </div>
                  <div>
                    <label class="block text-xs font-bold text-text-muted uppercase mb-1">Email Address</label>
                    <input v-model="form.managers[index].email" required type="email" class="w-full bg-background border border-border rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50" placeholder="manager@example.com" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="flex justify-between pt-4">
              <button @click="prevStep" class="text-text-muted hover:text-text-main px-4 py-2 font-medium">← Back</button>
              <button @click="nextStep" :disabled="!managersValid" class="bg-primary text-white px-6 py-3 rounded font-bold hover:bg-primary-hover transition disabled:opacity-50">Next: Admin Account →</button>
            </div>
          </div>
          ...
          <div v-if="step === 5" class="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
            ...
            <div class="space-y-4">
              <div>
                <label class="block text-xs font-bold text-text-muted uppercase mb-1">Full Name</label>
                <input v-model="form.adminName" required class="w-full bg-background border border-border rounded px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50" placeholder="Admin Name" />
              </div>
              <div>
                <label class="block text-xs font-bold text-text-muted uppercase mb-1">Email Address</label>
                <input v-model="form.adminEmail" required type="email" class="w-full bg-background border border-border rounded px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50" placeholder="admin@example.com" />
              </div>
              <div>
                <label class="block text-xs font-bold text-text-muted uppercase mb-1">Password</label>
                <input v-model="form.adminPassword" required type="password" class="w-full bg-background border border-border rounded px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50" placeholder="••••••••" />
              </div>
            </div>

            <div class="flex justify-between pt-4">
              <button @click="prevStep" class="text-text-muted hover:text-text-main px-4 py-2 font-medium">← Back</button>
              <button @click="nextStep" :disabled="!form.adminName || !form.adminEmail || form.adminPassword.length < 8" class="bg-primary text-white px-6 py-3 rounded font-bold hover:bg-primary-hover transition disabled:opacity-50">Review & Ignite →</button>
            </div>
          </div>
          ...
          <div v-if="step === 6" class="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
            <div v-if="!ignitionComplete">
              ...
              <div class="bg-gray-50 p-4 rounded border border-border text-sm space-y-2 mb-6">
              ...
              <div v-if="error" class="text-ember text-sm mb-4 bg-red-50 p-3 rounded border border-red-100">{{ error }}</div>
              ...
              <div class="flex justify-between items-center pt-4 border-t border-border">
                ...
                <button @click="igniteSystem" :disabled="igniting" class="bg-gradient-to-r from-primary to-blue-600 text-white px-8 py-4 rounded font-black text-lg hover:shadow-lg hover:shadow-primary/30 transition-all disabled:opacity-50 flex items-center gap-2">
            ...
            <div v-else class="space-y-6">
              ...
              <div v-if="managerInvites.length > 0" class="bg-surface border border-border rounded p-6">
                ...
                <div class="space-y-3">
                  <div v-for="invite in managerInvites" :key="invite.email" class="flex flex-col gap-1 p-3 bg-gray-50 rounded border border-border">
              ...
              <div class="pt-6 flex justify-center">
                <button @click="finishSetup" class="bg-primary text-white px-8 py-3 rounded font-bold hover:bg-primary-hover transition text-lg w-full">
                  Go to Login →
                </button>
              </div>
            </div>
